/*
# Fix restore_project_backup: stage_checklist + strict supplier validation

## Problem 1: Stage checklist not restored
The backup stores checks as booleans on Stage (checkServico, checkConferido, etc.)
but the DB uses stage_checklist rows. The RPC didn't materialize these.
Now after creating each stage, 5 stage_checklist rows are inserted from the booleans.

## Problem 2: Invalid supplier silently becomes NULL
Materials/equipment with a non-existent supplier_id were silently set to NULL.
Now: if supplierId is provided but not found in the supplier map, RAISE EXCEPTION.

## Other validation tightened
- Stage dependency: if provided but not in stage map, RAISE EXCEPTION
- All exceptions cause full transaction rollback (atomic)

## Strategy for stage_checklist
- Legacy boolean format (current): convert checkServico etc. to 5 rows
- If backup has a "stageChecklist" array with normalized rows, use those instead
- Never create both
*/
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      v_supplier_map := v_supplier_map || jsonb_build_object(v_supplier->>'id', v_new_supplier_id::text);
      v_count := v_count + 1;
    END LOOP;

    -- Stages (no deps yet, no profissional yet)
    ...
    LOOP
      ...
      v_stage_map := v_stage_map || jsonb_build_object(v_stage->>'id', v_new_stage_id::text);
      v_count := v_count + 1;

      -- Stage checklist: prefer normalized array, fallback to legacy booleans
      v_cl_items := v_stage->'stageChecklist';
      v_has_normalized_cl := v_cl_items IS NOT NULL AND jsonb_array_length(COALESCE(v_cl_items, '[]'::jsonb)) > 0;

      IF v_has_normalized_cl THEN
        -- Normalized format: use provided checklist rows
        FOR v_cl_row IN SELECT * FROM jsonb_array_elements(v_cl_items)
        LOOP
          INSERT INTO stage_checklist (project_id, stage_id, label, completed, sort_order)
          VALUES (
            v_project_id, v_new_stage_id,
            COALESCE(v_cl_row->>'label', ''),
            COALESCE((v_cl_row->>'completed')::boolean, false),
            COALESCE((v_cl_row->>'sort_order')::integer, 0)
          );
          v_count := v_count + 1;
        END LOOP;
      ELSE
        -- Legacy boolean format: convert 5 booleans to rows
        INSERT INTO stage_checklist (project_id, stage_id, label, completed, sort_order)
        VALUES
          (v_project_id, v_new_stage_id, 'Servico executado', COALESCE((v_stage->>'checkServico')::boolean, false), 1),
          (v_project_id, v_new_stage_id, 'Servico conferido', COALESCE((v_stage->>'checkConferido')::boolean, false), 2),
          (v_project_id, v_new_stage_id, 'Ambiente limpo', COALESCE((v_stage->>'checkLimpo')::boolean, false), 3),
          (v_project_id, v_new_stage_id, 'Pagamento conferido', COALESCE((v_stage->>'checkPagamento')::boolean, false), 4),
          (v_project_id, v_new_stage_id, 'Sem pendencias', COALESCE((v_stage->>'checkPendencias')::boolean, false), 5);
        v_count := v_count + 5;
      END IF;
    END LOOP;

    -- Update stage dependencies (strict validation)
    FOR v_stage IN SELECT * FROM jsonb_array_elements(COALESCE(v_proj->'obra', '[]'::jsonb))
    LOOP
      IF v_stage->>'dependencia' IS NOT NULL AND v_stage->>'dependencia' <> '' THEN
        v_new_stage_id := (v_stage_map ->> (v_stage->>'id'))::uuid;
        v_dep_id := (v_stage_map ->> (v_stage->>'dependencia'))::uuid;
        IF v_dep_id IS NULL THEN
          RAISE EXCEPTION 'Etapa "%" referencia dependencia inexistente: %', v_stage->>'nome', v_stage->>'dependencia';
        END IF;
        UPDATE stages SET dependencia = v_dep_id WHERE id = v_new_stage_id;
      END IF;
    END LOOP;
    ...
    LOOP
      ...
      v_prof_map := v_prof_map || jsonb_build_object(v_prof->>'id', v_new_prof_id::text);
      v_count := v_count + 1;
    END LOOP;

    -- Update stage profissional_id (strict validation)
    FOR v_stage IN SELECT * FROM jsonb_array_elements(COALESCE(v_proj->'obra', '[]'::jsonb))
    LOOP
      IF v_stage->>'profissionalId' IS NOT NULL AND v_stage->>'profissionalId' <> '' THEN
        ...
        IF v_new_prof_id IS NULL THEN
          RAISE EXCEPTION 'Etapa "%" referencia profissional inexistente: %', v_stage->>'nome', v_stage->>'profissionalId';
        END IF;
        UPDATE stages SET profissional_id = v_new_prof_id WHERE id = v_new_stage_id;
      END IF;
    END LOOP;

    -- Jobs (strict validation for stage + professional)
    ...
    LOOP
      ...
      IF v_new_stage_id IS NULL THEN
        RAISE EXCEPTION 'Trabalho referencia etapa inexistente: %', v_job->>'etapa_id';
      END IF;
      v_new_prof_id := (v_prof_map ->> (v_job->>'profissional_id'))::uuid;
      IF v_new_prof_id IS NULL THEN
        RAISE EXCEPTION 'Trabalho referencia profissional inexistente: %', v_job->>'profissional_id';
      END IF;
      INSERT INTO jobs (
    ...
    -- Materials (strict validation for stage + supplier when provided)
    ...
    LOOP
      ...
      IF v_new_stage_id IS NULL THEN
        RAISE EXCEPTION 'Material "%" referencia etapa inexistente: %', v_material->>'nome', v_material->>'etapa_id';
      END IF;
      v_new_supplier_id := NULL;
      IF v_material->>'fornecedorId' IS NOT NULL AND v_material->>'fornecedorId' <> '' THEN
        v_new_supplier_id := (v_supplier_map ->> (v_material->>'fornecedorId'))::uuid;
        IF v_new_supplier_id IS NULL THEN
          RAISE EXCEPTION 'Material "%" referencia fornecedor inexistente: %', v_material->>'nome', v_material->>'fornecedorId';
        END IF;
      END IF;
      INSERT INTO materials (
        ...
        v_project_id, v_new_stage_id, v_new_supplier_id,
    ...
    -- Equipment (strict validation for stage + supplier when provided)
    ...
    LOOP
      ...
      IF v_new_stage_id IS NULL THEN
        RAISE EXCEPTION 'Equipamento "%" referencia etapa inexistente: %', v_equip->>'nome', v_equip->>'etapa_id';
      END IF;
      v_new_supplier_id := NULL;
      IF v_equip->>'fornecedorId' IS NOT NULL AND v_equip->>'fornecedorId' <> '' THEN
        v_new_supplier_id := (v_supplier_map ->> (v_equip->>'fornecedorId'))::uuid;
        IF v_new_supplier_id IS NULL THEN
          RAISE EXCEPTION 'Equipamento "%" referencia fornecedor inexistente: %', v_equip->>'nome', v_equip->>'fornecedorId';
        END IF;
      END IF;
